...
class Strategy:
    def reset_state(self) -> None:
        ...
        self.gamma: float = 0.005            # risk aversion
        self.inv_cap: float = 120.0          # position cap
    ...
    # -------- Liquidity check (fixed) --------
    def _sufficient_liquidity(self, min_qty: float = 10.0) -> Tuple[bool, bool]:
        """
        Returns (bid_ok, ask_ok) using safe defaults if sizes are None.
        Uses half the threshold per side to avoid over-vetoing.
        """
    ...
    def _maybe_quote(self, t_left: Optional[float]) -> None:
        ...
        bid_ok, ask_ok = self._sufficient_liquidity(min_qty=10.0)
